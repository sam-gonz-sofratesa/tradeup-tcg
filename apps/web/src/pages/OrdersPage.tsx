  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link to="/dashboard" className="text-sm text-[var(--color-muted)] hover:text-white transition-colors">
          ← Dashboard
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <Link to="/store" className="mt-4 inline-block text-sm text-[var(--color-brand)] hover:underline">
            Ir a la tienda →
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
          {transactions.map((tx) => {
            ...
            const date = new Date(tx.createdAt).toLocaleDateString('es-PA', {
            ...
            return (
              <div
                key={tx._id}
                className="rounded-xl bg-[var(--color-surface-1)] border border-[var(--color-border)] p-4 flex items-center gap-4"
              >
                {/* Icono tipo */}
                <div className="w-10 h-10 rounded-lg bg-[var(--color-surface-3)] flex items-center justify-center text-xl shrink-0">
                  {isB2C ? '🏪' : tx.type === 'c2c_trade' ? '🔄' : '💸'}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[var(--color-muted)] text-xs mt-0.5 truncate">
                    {date} · ID: {tx._id.slice(-8)}
                  </p>
                  {tx.stripePaymentIntentId && (
                    <p className="text-[var(--color-muted)] text-xs font-mono truncate">
                      pi_{tx.stripePaymentIntentId.slice(-12)}
                    </p>
                  )}
                </div>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-3 mt-6">
          <button
            ...
            disabled={page === 1}
            className="px-4 py-2 text-sm rounded-lg border border-[var(--color-border)] text-[var(--color-muted)] hover:text-white disabled:opacity-40 transition-colors"
          >
            ← Anterior
          </button>
          <span className="text-[var(--color-muted)] text-sm self-center">
            {page} / {totalPages}
          </span>
          <button
