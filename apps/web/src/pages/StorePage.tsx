  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="mb-8">
        ...
        <p className="text-[var(--color-muted)] text-sm">Singles y sellados con garantía TradeUp</p>
      </div>
      {isLoading && (
...
function StoreCard({ item }: { item: any }) {
  // Prioridad: foto subida → imageUrl del catálogo → placeholder
  const image = item.photos?.[0] ?? item.catalogCard?.imageUrl ?? null

  return (
    <div className="rounded-[var(--radius-card)] bg-[var(--color-surface-2)] border border-[var(--color-border)] overflow-hidden card-glow">
