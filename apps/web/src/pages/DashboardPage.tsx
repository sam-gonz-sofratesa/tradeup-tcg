    ...
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-10">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[var(--color-muted)] text-sm mt-1">⭐ {user?.reputation?.toFixed(1) ?? '–'} · {user?.reviewCount ?? 0} reviews · rol: {user?.role}</p>
        </div>
        {user?.stripeConnectStatus !== 'active' && (
          <button onClick={() => stripeOnboard.mutate()} disabled={stripeOnboard.isPending}
        ...
        <section>
          ...
          <div className="space-y-3">
            {offersReceived.map((offer: any) => (
              <div key={offer._id} className="flex items-center justify-between p-4 rounded-xl bg-[var(--color-surface-2)] border border-[var(--color-border)]">
                <div>
                  <p className="text-sm text-white">
                    <span className="font-medium">{offer.buyer?.username}</span> ofrece{' '}
                    {offer.type === 'money' ? `$${(offer.moneyAmount / 100).toFixed(2)}` : offer.type === 'cards' ? 'cartas' : 'oferta mixta'}
                  </p>
                  <p className="text-xs text-[var(--color-muted)] mt-0.5">por: {offer.listing?.catalogCard?.name ?? 'tu carta'}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => acceptOffer.mutate(offer._id)}
                    className="px-3 py-1.5 rounded-lg bg-green-600/20 border border-green-600/40 text-green-400 text-xs hover:bg-green-600/30 transition-all">Aceptar</button>
                  <button onClick={() => declineOffer.mutate(offer._id)}
                    className="px-3 py-1.5 rounded-lg bg-red-600/20 border border-red-600/40 text-red-400 text-xs hover:bg-red-600/30 transition-all">Rechazar</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {listings.length === 0
          ...
          : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {listings.map((l: any) => (
                <div key={l._id} className="p-3 rounded-xl bg-[var(--color-surface-2)] border border-[var(--color-border)]">
                  ...
                  {l.price && <p className="text-sm font-semibold text-[var(--color-brand-light)] mt-1">${(l.price / 100).toFixed(2)}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        <section>
          ...
          <div className="space-y-2">
            {offersSent.map((offer: any) => (
              <div key={offer._id} className="flex items-center justify-between p-3 rounded-xl bg-[var(--color-surface-2)] border border-[var(--color-border)]">
                <p className="text-sm text-[var(--color-muted)]">
                  {offer.listing?.catalogCard?.name ?? 'carta'} ·{' '}
                  <span className={offer.status === 'pending' ? 'text-yellow-400' : offer.status === 'accepted' ? 'text-green-400' : 'text-red-400'}>
                    {offer.status}
                  </span>
                </p>
              </div>
