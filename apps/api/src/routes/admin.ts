...
adminRoutes.patch('/users/:id/role', requireAdmin, async (c) => {
  ...
  // Sync role to Clerk public_metadata so JWT reflects the change
  try {
    ...
    // Non-fatal: MongoDB is source of truth; Clerk will reflect on next token refresh
  }
